...
app.post("/entrada", (req,res)=>{
  ...
  db.run("INSERT INTO registros(user_id,entrada) VALUES(?,?)", [userId, ahora], function(err){
    if(err) return res.status(500).send({error:err.message});
...
app.post("/salida", (req,res)=>{
  ...
  db.get("SELECT * FROM registros WHERE user_id=? AND salida IS NULL ORDER BY id DESC LIMIT 1", [userId], (err,row)=>{
    if(err) return res.status(500).send({error:err.message});
    if(!row) return res.status(400).send({error:"No hay turno activo"});
    const ahora = new Date().toISOString();
    db.run("UPDATE registros SET salida=? WHERE id=?", [ahora,row.id], function(err){
      if(err) return res.status(500).send({error:err.message});
      res.send({success:true});
    });
  });
...
app.get("/registros", (req,res)=>{
  db.all(`SELECT r.id, u.nombre, r.entrada, r.salida 
          FROM registros r 
          JOIN users u ON r.user_id = u.id
          ORDER BY r.entrada DESC`, [], (err, rows)=>{
    if(err) return res.status(500).send({error:err.message});
    res.send(rows);
  });
});
